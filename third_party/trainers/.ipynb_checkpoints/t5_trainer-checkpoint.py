...
class T5Trainer(Trainer):
    ...
    def _ins_ins_loss(self, features, indices):
        ...
        labels = indices.contiguous().view(-1, 1)
        ...
        log_probs = logits - torch.log(denominator)  # B * B
    ...
    def _pro_ins_loss(self, features, indices, task_embed):
        device = (torch.device('cuda')
                  if features.is_cuda
                  else torch.device('cpu'))
        features = F.normalize(features, p=2, dim=1)
        task_embed = F.normalize(task_embed, p=2, dim=1)
        anchor_dot_contrast = torch.div(
            torch.matmul(features, task_embed.T),
            0.1)  # 计算两两样本间点乘相似度
        # for numerical stability
        # logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        # logits = anchor_dot_contrast - logits_max.detach()
        ...
        denominator = torch.sum(exp_logits, axis=1, keepdim=True) - exp_logits[range(features.shape[0]), indices].unsqueeze(-1)
        fenzi = anchor_dot_contrast[range(features.shape[0]), indices].unsqueeze(-1)  # index:任务的索引,[batch]
        log_probs = fenzi - torch.log(denominator)
    ...
    def _compute_loss(self, model, inputs, labels):
        if self.args.label_smoothing == 0:
            ...
            # loss = loss + (self._ins_ins_loss(result.sample2, result.indices) \
            #        # + self._ins_ins_loss(result.sample3, result.indices) \
            #        + self._pro_ins_loss(result.sample2, result.indices, result.task_embed1) \
            #        + self._pro_ins_loss(result.sample2, result.indices, result.task_embed2))
            loss = loss + (self._ins_ins_loss(result.sample2, result.indices) \
                   + self._pro_ins_loss(result.sample2, result.indices, result.task_embed2))
            # loss = loss + (self._ins_ins_loss(result.sample2, result.indices))
            # loss = loss + self._pro_ins_loss(result.sample2, result.indices, result.task_embed2)
    ...
    def evaluate(
        self, eval_datasets: Optional[Dict[str, Dataset]] = None, ignore_keys=None, metric_key_prefix = None
    ) -> Dict[str, float]:
        """
