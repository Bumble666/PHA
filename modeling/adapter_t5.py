...
class Seq2SeqLMOutput(Seq2SeqLMOutput):
    ...
    sample1 : Optional[Tuple[torch.FloatTensor]] = None
    indices: Optional[Tuple[torch.FloatTensor]] = None
    task_embed1: Optional[Tuple[torch.FloatTensor]] = None
    sample2 : Optional[Tuple[torch.FloatTensor]] = None
    task_embed2: Optional[Tuple[torch.FloatTensor]] = None
    sample3: Optional[Tuple[torch.FloatTensor]] = None
...
class T5StackWithAdapter(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        if (self.is_decoder and self.config.decoder_adapter == "generated") or (
            (not self.is_decoder) and self.config.encoder_adapter == "generated")or(
            self.is_decoder and self.config.decoder_adapter == "pro-hyper") or (
                (not self.is_decoder) and self.config.encoder_adapter == "pro-hyper"
        ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.is_decoder and self.config.decoder_adapter == "generated":
        ...
        elif self.is_decoder and self.config.decoder_adapter == "pro-hyper":
            global sample2
            sample2 = self.mlp(
                        mean_pooling(
                            encoder_hidden_states, kwargs["encoder_attention_mask"]
            ...
            global taskemb
            temp = torch.arange(0, len(self.config.tasks), device=input_ids.device).long()
            taskemb = self.adapter_task_embedding(temp)
            task_embed2 = self.adapter_task_embedding(indices)  # task_num * hid_dim
            self.apply_params_to_adapters(input_ids.size(0), self.param_gen(task_embed2))
